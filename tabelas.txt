...
BEGIN
    BEGIN
        ...
        DBMS_OUTPUT.PUT_LINE('Registro de farm�cia inserido com sucesso.');
    EXCEPTION
...
DECLARE
   ...
   CURSOR especialidade_cur IS
      SELECT es.cod_especialidade, es.especialidade, es.medico_cod_medico
      FROM especialidade es;
BEGIN
   FOR especialidade_rec IN especialidade_cur
   LOOP
      ...
      DBMS_OUTPUT.PUT_LINE('Codigo da Especialidade: ' || v_cod_especialidade);
      DBMS_OUTPUT.PUT_LINE('Especialidade: ' || v_especialidade);
      DBMS_OUTPUT.PUT_LINE('C�digo do M�dico: ' || v_cod_medico);
...
DECLARE
   ...
   v_observacao         VARCHAR2(30);
   v_segmento_acao      VARCHAR2(30);
   v_triagem_cod_triagem NUMBER;

   CURSOR medico_diagnostico_cur IS
      SELECT m.COD_MEDICO,
             m.CRP,
             m.PESSOA_FISICA_CPF,
             d.COD_DIAGNOSTICO,
             ...
             d.OBSERVACAO,
             d.SEGMENTO_ACAO,
             d.TRIAGEM_COD_TRIAGEM
      FROM MEDICO m
           JOIN DIAGNOSTICO d ON m.COD_MEDICO = d.MEDICO_COD_MEDICO;
BEGIN
   FOR medico_diagnostico_rec IN medico_diagnostico_cur
   LOOP
      ...
      v_observacao         := medico_diagnostico_rec.OBSERVACAO;
      v_segmento_acao      := medico_diagnostico_rec.SEGMENTO_ACAO;
      v_triagem_cod_triagem := medico_diagnostico_rec.TRIAGEM_COD_TRIAGEM;

      DBMS_OUTPUT.PUT_LINE('Codigo do Medico: ' || v_cod_medico);
      DBMS_OUTPUT.PUT_LINE('CRP: ' || v_crp);
      DBMS_OUTPUT.PUT_LINE('CPF da Pessoa Fesica: ' || v_pessoa_fisica_cpf);
      DBMS_OUTPUT.PUT_LINE('C�digo do Diagn�stico: ' || v_cod_diagnostico);
      DBMS_OUTPUT.PUT_LINE('Data do Diagn�stico: ' || TO_CHAR(v_data_geracao, 'DD/MM/YYYY'));
      DBMS_OUTPUT.PUT_LINE('Resultado do Diagn�stico: ' || v_resultado);
      DBMS_OUTPUT.PUT_LINE('Observa��o: ' || v_observacao);
      DBMS_OUTPUT.PUT_LINE('Segmento de A��o: ' || v_segmento_acao);
      DBMS_OUTPUT.PUT_LINE('C�digo da Triagem: ' || v_triagem_cod_triagem);
      DBMS_OUTPUT.PUT_LINE('----------------------------------------');
   END LOOP;
END;
/
